...
// ============================================================
// 閿熻�璇ф嫹, 鑹叉牎, , , 閿熻娇??鎷? , 鎶? , , 榫? , , , , , 棣? 閿?
// ,  HSV 閿熺Ц纭锋嫹, , 閿熺纯 閫? , 閿熸垝锛孲/V 閫? ,  [1-strength, 1+strength] , 鍥? , , , 閿?
// ============================================================
// 閿熻�璇ф嫹, 鑹插井, , ,  LAB 閿熺Ц纭锋嫹閿熻娇??鎷烽敓?L, , 閿熼ズ锝忔嫹閫?
// LAB , 鐭? 閿熼ズ锝忔嫹, ,  L , ,  AB ,  , 鑹? 閿熸垝銆? 鎽? ,
// L , 鍥? [-strength, +strength] 鍋? , , , 娴掔�鎷烽敓琛??鎷烽敓?
                             ...
                             const Config& config)
{
    ...
    double msPrep = 0;  // DB, ,  + GPU library, , , , GPU璺? ,

    // , , , 鍙? , ,  profile, , 閿熻�绮鹃敓楗虹�鎷峰師, 閿熸�纭锋嫹, ,
    ...
    // , , 鐩? 鍥? 閿熸嵎鐧告嫹甯? , 閿熸埅璇ф嫹, , , , 閿熻妭闈╂嫹, 鍘婚敓鎴�綇鎷?
    std::string targetHash;
    {
        // 姣?100 , 閿熸埅璇ф嫹,  1 , , , 閿熼ズ?10000 , ,  ,  3 閫?  ,  30KB
    ...
    // 鎸? , , 鍙介敓缁炴唻鎷? , , 閿熶茎鍖℃嫹閿熼叺纭锋嫹, , 璋嬮敓?tile , , , , 閿?tile 濮? 鍘? 閿熻�鎲嬫嫹閿熺粸锝忔嫹
    ...
    // --upscale, 閿熻剼杈炬嫹鍘熷浘, 鍙? ,  tile, 鍚? , 鐩? 鍓? , , 鑺忛緥閿?
    if (cfg.upscale > 1)
    {
        cv::Mat up;
    ...
    // Read feature resolution from DB meta (required; old DBs must be rebuilt) , 閿熸嵎鏃у簱锛?
    ...
    // 閼??濮╅幒銊??鏉堟挸鍤?tile閿涙碍铆楠?閳?320鑴?80閿涘瞼鐝�獮?閺傜懓鑸?閳?180鑴?20
    ...
    // , 鍙? , 鐩�綍, ,  FeaturePack / ANN 閿熻�涔呬紮鎷蜂娇閿熺煫锝忔嫹
    std::string featDirCache;
    auto allRecords = db.allRecords();  // 鍏? 閿熼摪纭锋嫹, 閿?GPU 璺? 閿熷彨甯�嫹, , 鍙?
    ...
    // , , ,  鍥? , , , 椹?GPU, , 閿熺�绋嬭�鎷烽敓鍙�》鎷?  tiny/LBP 閿熶茎纭锋嫹,  , , ,
    ...
    if (cfg.useGpu && cuda::isCudaAvailable())
    {
        ...
        // , 閿熺殕纭锋嫹閿熸埅璁规嫹, , , , , 閿熻姤锛坱iny.bin + lbp.bin,
        // , , , 鏁堟椂,  2 ,  fread , 閿?50K , 閿熶茎纭锋嫹 I/O
        ...
        if (!cacheLoaded)
        {
            // , 閿熻姤涓? 閿熻妭浼欐嫹澶辨晥 ,  , 閿熷壙纰夋嫹, 閿熺�绛规嫹, 閿熶茎纭锋嫹, 鍙?
            ...
            if (nUploadThreads > 16) nUploadThreads = 16;  // , ,  I/O 閿熺�绋嬬櫢鎷烽敓娲佸弽, 閿熷壙浼欐嫹
            std::vector<std::thread> uploadWorkers;
            for (int t = 0; t < nUploadThreads; ++t)
            {
                uploadWorkers.emplace_back([&, t]() {
                    FeatureCache cache;  // 姣? 閿熺�绋嬭�鎷? , 閿熻姤锛? , , 閿?
    ...
    // WebP , ,  16383px ,  閿熼ズ鎲嬫嫹, 閿熻剼锝忔嫹, , 寮?JPG 閿熺�纭锋嫹涓€,
    ...
    // 婧㈠嚭妫€鏌ワ細澶у瀷椹�禌鍏嬪彲鑳借秴杩?int32 鑼冨洿
    ...
    // 瀹介珮姣旀牎楠岋細DB 鐗瑰緛绌洪棿 vs 杈撳嚭 tile锛宖eatW/featH 浠巑eta瑙ｆ瀽鑰屾潵
    if (featH <= 0 || featW <= 0) {
    ...
    // , 缂? 鍏? , 鐩?  tile 閿熺�杈炬嫹,  pad, , , , 閿?tile 閿熺�瀵革級
    ...
    std::vector<int>    analyzeRanks;     // winner 閿熻妭鐚存嫹閫? , 閿熷彨纰夋嫹浣? (1-based)
    std::vector<int>    analyzeAnnRanks;  // winner ,  ANN Top200 閿熷彨纰夋嫹浣? (0=, , )
    std::vector<int>    analyzeCat;       // 0=Smooth, 1=Edge, 2=Texture, 3=Normal
    double analyzeGridCellSum[64] = {0};   // 姣?  cell 閿熶茎鎾呮嫹, 閿熸�璁★綇鎷? 閿熻妭鐧告嫹閿熼樁鍑ゆ嫹, ,
    FeatureCache analysisFeatureCache;

    int N = cfg.candidates;  // , 閫? , GPU 璺? , , ,  benchmark,

    // Benchmark , ,  lambda, 閿熻妭鏂ゆ嫹, 鏃? 閿熺煫锝忔嫹, , ,  totalTiles/N 涔? 閿熻棄锛?
    ...
    // Phase D , , ,  4, 4 閿熺殕鎲嬫嫹閿熺煫锝忔嫹8, 8, 4, 4,

    int nThreads = std::thread::hardware_concurrency();
    if (nThreads < 2) nThreads = 2;

    // 鍑? 閿熼樁娈电》鎷锋椂, DB, 閿熸埅鈽呮嫹GPU library, , , 閿熷壙鏂ゆ嫹,
    auto tPreFeat = Clock::now();
    msPrep = Ms(tPreFeat - tLast).count();
    tLast = tPreFeat;

    // Phase 0: , , , 鍙? GPU , , , 閿熷姭锝忔嫹CPU , 閿熷壙锝忔嫹
    if (cfg.useGpu)
    {
        const int BATCH = 256;
        ...
        for (; batchStart + BATCH <= totalTiles; batchStart += BATCH)
        {
            ...
            // CPU resize: tile ,  featW, featH, , 閿熺�绋嬶綇鎷?
    ...
    if (!cfg.useGpu)  // CPU , 閿熷壙锝忔嫹, ,  16 閿熺�绛规嫹, 鍙?
    {
    ...
    // , 閿熸触绐楀尅鎷烽敓鐨嗚�鎷? , 閿熷姭闈╂嫹,  2 ,  tile, , 鐩? 閿熸触锛夌尨鎷烽粯,  300, 姘村钩, ,
    ...
    // 寮洪敓鐙＄》鎷? 鍚屼竴鍥剧墖, 閿熷姭纭锋嫹閿?minGap ,  tile , , 閿熷姭杈炬嫹浣?
    const int MIN_GAP = std::max(50, tilesX);  // , , 涓€,
    std::unordered_map<int, int> lastUsedAt;   // imageId ,  , 閿熺粸鐧告嫹鐜�敓?tile , 閿?
    std::deque<std::vector<float>> recentGrids;  // , 閿熼叺纭锋嫹閿熻В锛? , , , 閿?00, ,
    constexpr double GRID_DUP_THRESHOLD = 0.010;  // , 閿熻緝鏍硷細闈╂嫹灏忛敓渚ユ拝鎷烽敓璇�嵆, 涓洪敓鎴�潻鎷?
    constexpr double GRID_DUP_PENALTY = 200.0;     // , 閿熼叺纭锋嫹鑳? , 閿熷彨??鐧告嫹閿?00,
    constexpr int GRID_DUP_WINDOW = 50;            // 閿熸暀璁规嫹, 閿熻妭锝忔嫹, , , 閿熻剼锝忔嫹, , 涓€, ,  tile , , ,

    // 鏉冮敓鎴�櫢鎷蜂竴, , , ,  tile , 閿熺煫锝忔嫹
    ...
    // 姣?  tile , , 閫夐敓鍙�殑纭锋嫹褰? GPU 璺? 棰勯敓鑺ワ紝CPU 璺? , , , , 閿?
    std::vector<ImageRecord> bestRecords(totalTiles);
    std::vector<int> bestLibIdx(totalTiles, -1);

    // , 鍥? 閿熺粸鎲嬫嫹鎷囬敓?Mat, 閿熻�鍖℃嫹妯″紡, , 瑕? , , 閿熻妭杈炬嫹閿熺殕鎲嬫嫹, 鏀�娇閿熺煫锝忔嫹
    cv::Mat output;
    ...
    {
        // 閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴
        // GPU , , , 姘撮敓绔�綇鎷稴QLite 棰?  ,  涓€,  GPU ,  椤? 閫?  ,  , 閿熺�绛规嫹, 鍥?
        // 閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴閿熺祴

        // , ,  Phase A: ANN , , , , , , 閿?, ,
        // , 閿熼ズ纭锋嫹閿熸埅鎸佷箙浼欐嫹, , , build 鏃? 閿熻姤锛? , , , 閿熸触鏋勬枻鎷? , ,
        ...
        // , ,  Phase B: , 骞?  tile , , , GPU , 瑕? , 閿熻妭瀛樺竷閿熻�锝忔嫹 , ,
        ...
        // , ,  Phase C: , ,  GPU , ,  , ,
        // , , 搴旀潈閿熸埅锝忔嫹, ,  tile , , 閫? , , 棰勯敓鍊燂紙瀹? 閫?  --adaptive-weights,
        ...
        if (cfg.adaptiveWeights)
        {
            for (int ti = 0; ti < totalTiles; ++ti)
            {
                ...
                {
                    // Smooth: ,  LAB , 鑹? , ,  Grid, , 鎴? , 閿熸彮??鎷峰崰閿熺粨鏋?
                    tileLabW[ti] = 0.25;
                    tileGridW[ti] = 0.45;
                    tileTinyW[ti] = 0.20;
                    tileEdgeW[ti] = 0.05;
                ...
                {
                    // Edge-heavy: , , 閿熺粨鏋?> , 鑹? , 鍊?0.01 , ,  9, 16 灏?ROI,
                    tileLabW[ti] = 0.15;
                    tileGridW[ti] = 0.40;
                    tileTinyW[ti] = 0.25;
                    tileEdgeW[ti] = 0.15;
        ...
        int noCandidateCount = 0;  // , 甯岄敓閰电�鎷? 钘撻敓绐栤槄鎷烽敓?tile
        for (int ti = 0; ti < totalTiles; ++ti)
        {
            ...
            // 棰戦敓缁炲垎纭锋嫹閿熼叺鍑ゆ嫹, 1, 閿熺粨缃?, , , 閿?, 2, 閿熷彨鍑ゆ嫹, 3+, 閿熸埅鍑ゆ嫹(, , , )
            for (int j = 0; j < N; ++j)
            {
            ...
            // Top-N , 閿熺獤鈽呮嫹閿熺但opN , , , , 鏁? 閫? ,
            // , ,  8, 8 vs , , , 4, 4 閿熺殕姣旓綇鎷?  --analyze,  , ,
            if (cfg.analyze && validCount > 0)
            {
                ...
                for (int j = 0; j < N; ++j)
                {
                    ...
                    // GPU scores 閿熺獤鐚存嫹 8, 8 grid, 4, 4 , ,  = , 鍘?8, 8 , ,  + 4, 4 , ,
            ...
            // --analyze: , 褰曢€?  tile , , , , 閿熻�锛? , , , 澶? , 閿熺嫛銉�嫹, , , 閿?
            if (cfg.analyze)
            {
                ...
                // Top-K Gap: winner vs true best, , 閿熼叺鍑ゆ嫹, 鍘熷�, , 閿熺瓔锛?
                ...
                // ANN rank: winner ,  ANN , 璇? , 姊伴敓杞夸紮鎷烽敓?(0=, , )
                int annRank = -1;
                for (int j = 0; j < N; ++j)
                {
                ...
                // Grid 8, 8 姣?cell , 閿熼樁锝忔嫹閿熸�纭锋嫹閫夐敓鍙��纰夋嫹 cell LAB , ,
            ...
            lastUsedAt[chosenId] = ti;       // , 褰? 閿熺粸鐧告嫹閿熻娇浼欐嫹閿?
        ...
        if (cfg.tiledOutput)
        {
            // 閿熻�鍖℃嫹, , 閿熺煫?tile , , 閿熶茎纭锋嫹, 閿熺潾灏鸿揪鎷? 閿熺嫛锝忔嫹, , 閿?Mat
            std::error_code ec;
            ...
            for (int t = 0; t < nThreads; ++t) {
                tileWorkers.emplace_back([&, t]() {
                    ...
                    for (int ti = t; ti < totalTiles; ti += nThreads) {
                        ...
                        // DZI , 寮? {name}_files/{level}/{col}_{row}.jpg
        ...
        // --- 缁熶竴鍐? 妯″紡, 閿熺�锝忔嫹,  PNG/TIFF , 鏁? JPG ,  stream , 寮? , , ---
        // auto, PNG/TIFF , 閿熸嵎鍖℃嫹, 閿熻妭杈炬嫹 ,  batch/stream, JPG 榛? 鍏?
        // stream, 寮? , 寮? , 鍐欓敓鏁欙綇鎷? 閿熻妭瀛橈級
        // batch, 寮? 鍏? , , 閿熸彮浼欐嫹閿熷彨杈炬嫹閿?
        bool useStream = false;   // true=, 寮? false=鍏?
        ...
        if (isHeavyFormat && rawBytes > 500LL * 1024 * 1024)
        {
            ...
            {
                useStream = false;  // JPG 榛? 鍏? , , 閿熺殕璁规嫹閿熷彨浼欐嫹
            }
            else // auto, PNG/TIFF , 閿熸嵎鍖℃嫹, 閿熻妭杈炬嫹閿熺殕璁规嫹閫?
            {
#ifdef _WIN32
        ...
        // else: <500MB , 閿?PNG/TIFF ,  閿熺�鎲嬫嫹鍑嗚矾,
        ...
        {
            // PNG stream 妯″紡, , , 鍐欓敓鏁欙綇鎷烽敓鑺傝揪鎷锋劊?~162KB
            std::cout << "  (streaming mode ,  low memory)" << std::endl;
        ...
        std::atomic<int> placeNoCand{0};  // , 鏁? 閫? 閿熼摪纰夋嫹澶?
        std::atomic<int> placeLoadErr{0}; // 閿熶茎纭锋嫹, 鍙栧け,
        std::vector<std::thread> placeWorkers;
        ImageCache imgCache;  // 閿熺�绋嬪府鎷峰叏, 閿熻姤锛? , 閿熸埅闈╂嫹 imread
        for (int t = 0; t < nThreads; ++t)
        {
    ...
    else
    {
        ...
        for (int ti = 0; ti < totalTiles; ++ti)
        {
            ...
            for (int j = 0; j < (int)imgIds.size(); ++j) {
                ...
                auto gapIt = lastUsedAt.find(r.id);  // 寮洪敓鐙＄》鎷烽敓?
    ...
    std::cout << "Mosaic saved: " << outPath
              ...
              << ")"
              << std::endl;
    // 纭�繚鐗瑰緛璁℃暟鍣ㄦ�纭�紙鍚勮矾寰勫彲鑳藉湪return鍓嶆湭瀹屾暣璁剧疆锛?
